...
-- Schema: finance (jadwal & pembayaran)
...
CREATE TABLE leasing.leasing_product (
    ...
    bunga_flat     NUMERIC(5,2) NOT NULL,
...
CREATE TABLE leasing.leasing_contract (
    ...
    customer_id         BIGINT NOT NULL REFERENCES dealer.customers(customer_id) ON DELETE RESTRICT,
...
CREATE TABLE leasing.leasing_tasks_attributes (
    ...
    tasa_value    VARCHAR(255),
    tasa_status   VARCHAR(15) DEFAULT 'inprogress'
                  CHECK (tasa_status IN ('inprogress', 'pending', 'completed', 'cancelled')),
    tasa_task_id  BIGINT NOT NULL REFERENCES leasing.leasing_tasks(task_id) ON DELETE CASCADE,
    created_at    TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);

-- 5. payment_schedule <<finance>>
