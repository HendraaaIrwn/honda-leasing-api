...
('DP-RINGAN-24', 'DP Ringan 24 Bulan', 24, 10.00, 20.00, 1.20, 350000, TRUE),
...
('KTR-2026-001', '2026-01-15', '2026-02-01', '2026-03-01', 24,
 18975000, 3000000, 15975000, 18700000, 779166, 'active', 1, 1, (SELECT product_id FROM leasing.leasing_product WHERE kode_produk='DP-RINGAN-24')),
('KTR-2026-002', '2026-02-05', '2026-02-20', '2026-03-20', 36,
 28050000, 5000000, 23050000, 28000000, 777777, 'approved', 1, 9, (SELECT product_id FROM leasing.leasing_product WHERE kode_produk='BEBAS-1TH-36')),
('KTR-2026-003', '2026-02-08', NULL, NULL, 12,
 33400000, 8000000, 25400000, 28000000, 2333333, 'draft', 2, 12, (SELECT product_id FROM leasing.leasing_product WHERE kode_produk='SUPER-KILAT-12'));
...
('Catatan Wawancara', 'Penghasilan stabil, rumah sendiri', 'completed',
    (SELECT task_id FROM leasing.leasing_tasks WHERE task_name='Input Pengajuan & Unggah Dokumen')),
('Foto Selfie + KTP', 'https://storage/.../selfie_ktp.jpg', 'completed',
    (SELECT task_id FROM leasing.leasing_tasks WHERE task_name='Input Pengajuan & Unggah Dokumen'));
